...
struct LinearProber : public Prober<KeyType> {
    ...
    HASH_INDEX_T next() 
    {
        ...
        if(this->numProbes_ == this->m_) { // probes equal to table size
            return this->npos; 
        }
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
  // use auto to iterate through the table and delete each allocaion
  for (auto ptr : table_)
  {
    delete ptr;
  }
...
bool HashTable<K,V,Prober,Hash,KEqual>::empty() const
{
  ...
  for (auto ptr : table_)
  {
    // if it ever passes then the table is not empty
    if (ptr != nullptr && ptr->deleted == false)
    {
      return false;
    }
  }
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
  ...
  double lf = static_cast<double>(elementCount_) / CAPACITIES[mIndex_];
  if (lf > resizeAlpha_)
  {
    resize();
  }
  ...
  if (index == npos)
  {
    // throw std::logic_error if there is no free location
    throw std::logic_error("No free location can be found");
  }
  if (table_[index] == nullptr)
  {
    //update the old key value with new value
